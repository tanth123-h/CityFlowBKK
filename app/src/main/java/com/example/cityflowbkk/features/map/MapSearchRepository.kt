...
class MapSearchRepository(
    ...
    suspend fun autocomplete(
        ...
        val searchInput = buildString {
            append(query.trim())
            category.querySuffix?.let { append(it) }
        }

        val body = JSONObject()
            .put("input", searchInput)
            .put(
                "locationBias",
                JSONObject().put(
                    "circle",
        ...
        category.primaryTypes?.let { types ->
            body.put("includedPrimaryTypes", org.json.JSONArray(types))
        }
